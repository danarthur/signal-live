...
export const TimelineItem: React.FC<TimelineItemProps> = ({ item, isLast }) => {
  ...
  const iconBg = isWork ? 'bg-[#4A453E]/10' : isPersonal ? 'bg-emerald-500/10' : 'bg-blue-500/10';
  const iconColor = isWork ? 'text-[#4A453E]' : isPersonal ? 'text-emerald-600' : 'text-blue-600';
  const lineColor = 'bg-[#4A453E]/10';
  ...
  return (
    <div className="relative flex gap-4 pl-1 group min-h-[60px]">
      ...
      <div className="flex flex-col items-center">
        {/* The Dot/Icon */}
        <div className={`relative z-10 w-8 h-8 rounded-full ${iconBg} flex items-center justify-center border border-white/40 shadow-sm group-hover:scale-110 transition-transform duration-300`}>
      ...
      <div className="flex-1 pb-2 pt-1">
        <div className="flex justify-between items-start">
           <h4 className="text-sm font-semibold text-[#2C2824] group-hover:text-[#4A453E] transition-colors leading-tight">
             {displayText}
           </h4>
           <span className="text-[10px] font-mono text-[#8B8276] bg-white/50 px-1.5 py-0.5 rounded-md whitespace-nowrap ml-2">
             {item.time}
           </span>
        </div>
        
        {/* Subtext based on type */}
        <p className="text-xs text-[#8B8276] mt-1 line-clamp-1 opacity-80 group-hover:opacity-100 transition-opacity">
