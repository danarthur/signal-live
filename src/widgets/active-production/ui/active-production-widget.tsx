    ...
    <div className="h-64">
      <LiquidPanel hoverEffect className="h-full flex flex-col justify-between">
        <div className="flex justify-between items-start mb-4">
          <h2 className="text-xs font-medium text-ink-muted uppercase tracking-widest">Active Production</h2>
        ...
        <div className="flex flex-col gap-2">
          {loading ? (
            ...
            <div className="py-6 text-center text-xs text-ink-muted italic">No active productions</div>
          ) : (
            gigs.map((gig) => (
              <Link href={`/events/g/${gig.id}`} key={gig.id} className="block">
                <LiquidPanel hoverEffect className="group flex items-center justify-between !p-3 liquid-panel-nested">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className="text-sm text-ink">{gig.title ?? 'Untitled Production'}</span>
                      <span className="text-[10px] text-ink-muted uppercase tracking-wider">run of show</span>
                    </div>
                  </div>
